...
load_entrypoint_nginx(){
    ...
    if [ -f /root/admin/sweb/nginx/start.sh ]; then
        bash /root/admin/sweb/nginx/start.sh || log "ADVERTENCIA: Entrypoint Nginx falló, continuando..."
...
directorio_de_trabajo(){
    log "Cambiando directorio al proyecto NextJS..."

    if cd /root/admin/nodenext/proyectos/iawprojects; then
        log "Directorio cambiado a: $(pwd)"
    else
        log "ERROR: No se pudo cambiar al directorio del proyecto NextJS"
        exit 1
    fi
}

construir_y_arrancar(){
    log "Instalando dependencias NextJS..."
    
    npm install
    
    # Construir proyecto NextJS
    if npm run build; then
        log "Proyecto NextJS construido"
    else
        log "ERROR: Falló npm run build"
        exit 1
    fi
    
    # Arrancar NextJS en segundo plano
    log "Arrancando NextJS en segundo plano..."
...
main(){
    mkdir -p "$LOG_DIR"
    touch "$LOG_FILE"
    log "=== Iniciando contenedor NextJS ==="
    log "Fecha: $(date)"
    load_entrypoint_base
    load_entrypoint_postgre
    load_entrypoint_nginx
